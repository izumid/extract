...
def extract(query,db_config,debug=False):
	try: 
...
def base_generate(separated,query,db_config,debug,destination,basefile_name,day=0,last_month=0):
	...
	if separated == False:
		result_set = extract(query=query,db_config=db_config,debug=debug)
		...
		for i in range(last_month):
			...
			result_set = extract(query=sql_q,db_config=db_config,debug=debug)

			if result_set is not None and result_set != []:
				df = result_set_transform(result_set,column_name,debug)
						
				debug_code(debug,"Previous month query", sql_q)
				prefix = f"{start_date.strftime("%Y")}{start_date.strftime("%m").zfill(2)}"
...
def main():
	try:
		...
		match export_type:
			case 0: 
				base_generate(separated=False,query=query,db_config=config_json,debug=debug,destination=destination,basefile_name=basefile_name)
			case 1: 
				base_generate(separated=True,query=query,db_config=config_json,debug=debug,destination=destination,basefile_name=basefile_name,day=day,last_month=int(config["FILE"]["previous_month"]))
			case 2:
				index_where =  query.index("WHERE")
				query=(query[:index_where-1]+";")
				base_generate(separated=False,query=query,db_config=config_json,debug=debug,destination=destination,basefile_name=basefile_name,day=day,last_month=int(config["FILE"]["previous_month"]))
		
	except Exception as error:
		debug_code(debug,({"eror": f"Failed to generate file(s): {error}"}))
		log("log_error","'Saving base file'",datetime.datetime.today())

	duration(check_time,"Start Time",start)
	duration(check_time,"Duration",datetime.datetime.now()-start)
